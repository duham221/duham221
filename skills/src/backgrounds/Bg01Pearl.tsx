// BG 01 — Pearl Foundation
import React from 'react';
import {AbsoluteFill, useCurrentFrame, useVideoConfig} from 'remotion';
import {Vignette, Grid, Hairline, fadeIn} from './Shared';
import {rgba, rgb, SOFTGOLD, CHAMPAGNE, PORCELAIN, MIST, PEARL, DUSTGOLD, CHARCOAL} from './palette';

export const Bg01Pearl: React.FC = () => {
	const frame = useCurrentFrame();
	const {fps} = useVideoConfig();

	const t = (frame / fps) * Math.PI * 2;
	const drift = Math.sin(t / 6) * 0.18; // was 0.04 — now 18% sweep
	const breathe = Math.sin(t / 4) * 0.22; // was 0.03 — now 22% size pulse

	const cx = 62 + drift * 100;
	const s1 = 38 + breathe * 100;
	const s2 = 24 + breathe * 60;

	return (
		<AbsoluteFill
			style={{
				opacity: fadeIn(frame),
				background: [
					`radial-gradient(ellipse ${s2}% ${s2 * 1.1}% at ${cx}% 42%, ${rgba(CHAMPAGNE, 0.95)} 0%, transparent 100%)`,
					`radial-gradient(ellipse ${s1}% ${s1 * 1.2}% at ${cx}% 42%, ${rgba(SOFTGOLD, 0.80)} 0%, transparent 100%)`,
					`radial-gradient(ellipse 52% 52% at 30% 85%, ${rgba(PEARL, 0.9)} 0%, transparent 100%)`,
					`radial-gradient(ellipse 72% 72% at 10% 10%, ${rgba(PORCELAIN, 0.95)} 0%, transparent 100%)`,
					`radial-gradient(ellipse 82% 82% at 100% 100%, ${rgba(MIST, 0.9)} 0%, transparent 100%)`,
					rgb(PORCELAIN),
				].join(', '),
			}}
		>
			<Hairline x1={0.08} y1={0.85} x2={0.92} y2={0.85} color={DUSTGOLD} alpha={0.35} />
			<Grid color={CHARCOAL} alpha={0.035} spacing={120} />
			<Vignette intensity={0.18} />
		</AbsoluteFill>
	);
};
